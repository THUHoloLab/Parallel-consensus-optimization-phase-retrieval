function [x_est, loss_history, corr_history] = COPEforSim(y, mask, params, options)
...
for iter = 1:max_iter
    ...
    if mod(iter, display_iter) == 0
        ...
        if options.display_iter > 0
            figure(2)
             imshow(mat2gray([abs(Q1(x)), angle(Q1(x))]), [0, 1], 'border', 'tight');
            % 
            % Q1x = angle(Q1(z1));
            % Q2x = angle(Q1(z2));
            % Q3x = angle(Q1(z3));
            % Q4x = angle(Q1(x));
            % Q1xF = abs(fftshift(fft2((Q1(z1)))));
            % Q2xF = abs(fftshift(fft2((Q1(z2)))));
            % Q3xF = abs(fftshift(fft2((Q1(z3)))));
            % Q4xF = abs(fftshift(fft2((Q1(x)))));
            % 
            % subplot(331)
            % plot(angle(Q1x(384,256:512)))
            % subplot(332)
            % plot(angle(Q2x(384,256:512)))
            % 
            % subplot(333)
            % plot(angle(Q3x(384,256:512)))
            % subplot(334)
            % plot(log(1+(Q1xF(384,256:512))))
            % subplot(335)
            % plot(log(1+(Q2xF(384,256:512))))
            % 
            % subplot(336)
            % plot(log(1+(Q3xF(384,256:512))))
            % subplot(337)
            % plot(angle(Q4x(384,256:512)))
            % 
            % subplot(338)
            % plot(log(1+(Q4xF(384,256:512))))
...
% 最终傅里叶空间滤波
x_est = ifft2(fftshift(1 .* fftshift(fft2(x))));
end